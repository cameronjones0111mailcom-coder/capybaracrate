...
/// links to include) and various metadata without you needing to worry (much) about styling and layout.
fn page_configuration() -> PageConfiguration {
// Define a navigation bar with only two items, a link for user accounts,
...
// It allows you to describe HTML via Rust macros that allow you to dynamically generate content
// and Rust variables from surrounding scopes (as long as they implement the `Render` trait).
html! {
span {
(tr("nav-demonlist"))
}
...
let footer = Footer::new(html! {
    "© Copyright 2026 CapybaraCrate"
    br;
    "All rights reserved"
    br;
    "CapybaraCrate is in no way affiliated with RobTopGamesAB ® or pointercrate.com"
})
...
PageConfiguration::new("CapybaraCrate", nav_bar, footer)
    // Used for the HTML "author" meta tag
    .author("CapybaraCrate")
    // Used for the HTML "keywords" meta tag
    .keywords("Geometry Dash demon list, CapybaraCrate")
```
